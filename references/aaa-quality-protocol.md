...
| 回归守卫 | 测试基线、回归检测、零退化率 | 代码变更 + 基线 | 回归报告 + EvoScore | 仅回归维度 |
...
| 阶段五 回归防护 | 回归报告清晰度审查 | 零退化率展示、EvoScore 趋势图、门禁结论明确 | 对比完成后 |
...
| 原则 | 来源 | 在本协议的体现 |
|------|------|--------------|
| 演进优于快照 | SWE-CI | 迭代循环而非一次性通过 |
| 诊断先于修复 | brooks-lint | 视觉批评家先审查再反馈建议 |
| 分工优于全能 | SWE-CI 双智能体 | 扩展为四子代理，执行与审查分离 |
